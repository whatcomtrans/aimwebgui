...
function AIMServer(serverURL = "/") {
    ...
    this.login = function (username, password, version = 4, callback) {
        ...
        var req = this.requestFactory("login", version, token, params, function(api_response, server){
    ...
    this.logout = function (token, version, callback) {
        ...
        var req = this.requestFactory("logout", version, token, params, function(api_response, server) {
            if (api_response.success == 1) {
    ...
    this.get_devices = function (token, version, page, results_per_page, device_type, filter_d_name, filter_d_description, filter_d_location, sort, sort_dir, status, show_all, callback) {
        callback = (typeof callback === 'function') ? callback : function() {};
...
function AIMWorkspace(server, prefix) {
    this.server = server;
    ...
    // Load a filtered list of presets available for this workspace
    (function() {
        var server = this.server;
        var prefix = this.prefix;
        var workspace = this;

        server.get_presets(null, null, null, null, function(success, version, timestamp, erros, page, results_per_page, total_presets, count_presets, presets) {
            if (success) {
                for (var i = 0, count_presets; i < count_presets; i++) {
                    if (presets[i]) {
                        workspace.presets += presets[i];
                    }
                }
            } else {
